...
export const StudentSettings = () => {
  ...
  useEffect(() => {
    try {
      ...
      updateLoad(true);
      let url = `https://homebackend-3y7i.onrender.com/getStudentSettings?token=${userData.token}&adminId=${userData.sId}`;
      fetch(url)
      .then(res => res.json())
      .then(dat => {
        updateLoad(false);
        if(dat.status==200) {
          ...
          handleInputChange('cls', dat.data.cls);
          updateCls(dat.data.cls);
          setPhotoPreview(dat.data.pfp=="sample"?"":dat.data.pfp);
        //   dat.data.cls.map((e) => {
        //     addClass2(e);
        //   });
  ...
  const handleSave = () => {
    if((formData.cls>=4)&&(formData.cls<=12)) {
    let userData = JSON.parse(localStorage.getItem("userData"));
    updateLoad(true);
    let url = `https://homebackend-3y7i.onrender.com/updateStudentSettings?token=${userData.token}&adminId=${userData.sId}&updates=${JSON.stringify(formData)}`;
    fetch(url)
    .then(res => res.json())
    .then(dat => {
      updateLoad(true);
...
else alert("Range for class is 4-12!!!");
  };

  const handleDeleteAccount = () => {
    let userData = JSON.parse(localStorage.getItem("userData"));
    updateLoad(true);
    let url = `https://homebackend-3y7i.onrender.com/deleteStudentAcc?token=${userData.token}&adminId=${userData.sId}`;
    fetch(url)
    .then(res => res.json())
    .then(dat => {
      updateLoad(false);
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-lg border-2 border-blue-800/20 bg-white/80 backdrop-blur-sm text-gray-900 font-medium focus:border-blue-800 focus:outline-none transition-all duration-300"
                    placeholder="Enter your Class (4-12)"
                  />
                </div>
              </div>
